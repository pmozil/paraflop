...
public:
  Renderer(
      ...
      std::shared_ptr<buffer::UniformBuffer> m_uniformBuffer)
      : window(window), m_instance(m_instance), m_surface(m_surface),
        ...
        m_uniformBuffer(std::move(m_uniformBuffer)) {
    m_createSyncObjects();
  }
  ~Renderer() { cleanup(); }
...
private:
  ...
  std::shared_ptr<descriptor_set::DescriptorSetHandler> m_descriptorSetHandler;
  std::shared_ptr<buffer::VertexBuffer> m_vertexBuffer;
  std::shared_ptr<buffer::IndexBuffer> m_indexBuffer;
  std::shared_ptr<buffer::UniformBuffer> m_uniformBuffer;
