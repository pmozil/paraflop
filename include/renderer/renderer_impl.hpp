...
void Renderer<GraphicsPipeline>::m_recordCommandBuffers(uint32_t imageIndex) {
  ...
  vkCmdBindDescriptorSets(
      ...
      VK_PIPELINE_BIND_POINT_GRAPHICS, m_graphicsPipeline->pipelineLayout, 0, 1,
      &m_descriptorSetHandler->descriptorSets[m_currentFrame], 0, nullptr);

  vkCmdDrawIndexed(
      m_commandBuffer->commandBuffers[m_currentFrame],
      static_cast<uint32_t>(m_indexBuffer->size / sizeof(uint32_t)), 1, 0, 0,
      0);
