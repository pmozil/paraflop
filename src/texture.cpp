...
Texture2D::Texture2D(
    ...
    if (static_cast<bool>(useStaging)) {
        ...
        // type bits)
        vkGetBufferMemoryRequirements(*m_deviceHandler, stagingBuffer,
                                      &memReqs);
        ...
        VK_CHECK(vkAllocateMemory(*m_deviceHandler, &memAllocInfo, nullptr,
                                  &stagingMemory));
        VK_CHECK(vkBindBufferMemory(*m_deviceHandler, stagingBuffer,
                                    stagingMemory, 0));

        // Copy texture data into staging buffer
        uint8_t *data;
        VK_CHECK(vkMapMemory(*m_deviceHandler, stagingMemory, 0, memReqs.size,
                             0, (void **)&data));
        ...
        utils::setImageLayout(copyCmd, image,
        ...
        VK_CHECK(vkBindImageMemory(*m_deviceHandler, mappableImage,
                                   mappableMemory, 0));
        ...
        // Map image memory
        VK_CHECK(vkMapMemory(*m_deviceHandler, mappableMemory, 0, memReqs.size,
                             0, &data));
